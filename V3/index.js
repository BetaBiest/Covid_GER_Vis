...
async function script() {
  // *** Load stuff ***
  const geoDataR = d3.json(
    "https://raw.githubusercontent.com/AliceWi/TopoJSON-Germany/master/germany.json"
  );
  const popDataR = d3.dsv(
    ";",
    "data/de_einwohnerzahlen_kreise_12411-0015_slim.csv",
    function (d) {
      return {
        lkId: d.lkId,
  ...
  let berlin = geoData.objects.counties.geometries.find((geometrie) => {
    return geometrie.properties.name === "Berlin";
  });
  ...
  for (let i = geoData.objects.berlin.geometries.length; i; --i) {
    ...
    geoData.objects.counties.geometries[index].properties.districtType =
      "Stadtkreis";
  }
  ...
  for (let item of obj_to_merge) {
    geoData.objects.counties.geometries.splice(
      geoData.objects.counties.geometries.indexOf(item),
      1
    );
  }
  ...
  for (let feature of geoData.objects.counties.geometries) {
    ...
    if (prefix)
      feature.properties.name =
        prefix + feature.properties.name.replace(/ /g, "_");

    let index = nestedData.indexOf(
      nestedData.find((d) => {
        return d.key == feature.id;
      })
    );
    if (0 <= index)
      feature.properties.data = nestedData.splice(index, 1)[0].values;
    index = popData.indexOf(
      popData.find((d) => {
        return d.lkId == feature.id;
      })
    );
    if (0 <= index)
      feature.properties.population = popData.splice(index, 1)[0].population;
  }
  ...
  const src = labels.append("g").attr("transform", "translate(340,840)");
  ...
  const cSAxis = d3
    ...
    .tickSize(25);
  cS.append("g").attr("transform", "translate(0,0)").call(cSAxis);
  ...
  slider
    ...
    .on("input", function getResDate() {
      ...
      slider_label.datum(day).text(dayT);
      ref.text("Data: RKI (ArcGISHub) State: " + dayT);

      d3.select("#districts")
        .selectAll("path")
        .style("fill", (d, i) => {
          var obj = d.properties.data.find((x) => {
            return x.date.getTime() >= day.getTime();
          });
          ...
          return colorScale(
            d.properties.data[index].acute
              ? d.properties.population / d.properties.data[index].acute
  ...
  const districts = topojson.feature(geoData, geoData.objects.counties)
    .features;
